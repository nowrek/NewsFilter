...
public class SettingsFragment extends PreferenceFragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (pagesGroup != null) {
            ...
            for (PageConfigData page : appConfigData.getPageList()) {
                pagesGroup.addPreference(createNewUrlPreferenceScreen(page.getKey(), page.getName(), page.getPageUrl(), page.getJoinedTags()));
            }
        }
    }
    ...
    public void addData(PageConfigData data){
        ...
        if(existingPreference == null){
            PreferenceCategory pagesGroup = (PreferenceCategory) getPreferenceManager().findPreference("pages_group");
            if (pagesGroup != null) {
                pagesGroup.addPreference(createNewUrlPreferenceScreen(data.getKey(), data.getName(), data.getPageUrl(), data.getJoinedTags()));
            }
        }
    ...
    private void consolidatedChange(ChangeConfig newChange){
        ...
        if(oldChange == null){
           ...
           if(newChange.getChangeType() == ChangeConfig.ChangeType.Modified || newChange.getChangeType() == ChangeConfig.ChangeType.Removed){
                currentChanges.remove(oldChange);
                currentChanges.add(newChange);
            }
    ...
    private void setButtonsForMainScreen(){
        ...
        saveButton.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                slidingActivity.getConfiguration().processChanges(currentChanges);
                currentChanges = new ArrayList<>();
                return true;
            }
        });
        ...
        cancelButton.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                currentChanges = new ArrayList<>();
                FragmentManager fragmentManager = selfReference.getFragmentManager();
    ...
    private void setButtonsForNewPageScreen() {
        ...
        saveButton.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            @Override
            public boolean onPreferenceClick(Preference preference) {
                EditTextPreference namePreference = (EditTextPreference) getPreferenceManager().findPreference("name");
                EditTextPreference urlPreference = (EditTextPreference) getPreferenceManager().findPreference("url");
                EditTextPreference tagsPreference = (EditTextPreference) getPreferenceManager().findPreference("tags");
                ...
                if(name != null && url!= null && tags != null) {
                    ...
                    selfReference.addChange(new ChangeConfig<>(ChangeConfig.ChangeType.Added, new PageConfigData(key, name, url, tagsList)));
                    namePreference.setText("");
                    urlPreference.setText("");
                    tagsPreference.setText("");
                    PreferenceScreen screen = (PreferenceScreen)preference.getPreferenceManager().findPreference("new_page_key");
                    assert screen != null;
    ...
    private PreferenceScreen createNewUrlPreferenceScreen (String inKey, String inName, String inUrl, String inTags){
        ...
        theScreen.addPreference(createActionPreference(R.string.button_delete, new ConfigPageDeleteButtonAction(selfReference), inKey+BUTTON_DELETE_SUFFIX));
        theScreen.addPreference(createActionPreference(R.string.button_save, new ConfigPageSaveButtonAction(selfReference), inKey+BUTTON_SAVE_SUFFIX));
        theScreen.addPreference(createActionPreference(R.string.button_cancel, new ConfigPageCancelButtonAction(selfReference), inKey+BUTTON_CANCEL_SUFFIX));
        return theScreen;
    }
